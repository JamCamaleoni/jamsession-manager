 ...
 */
export const generateNextBand = (allUsers: User[], existingBands: Band[], pastBands: Band[] = [], forceSize?: number): Band | null => {
  ...
  pickMemberForRole(InstrumentType.DRUMS);
  pickMemberForRole(InstrumentType.BASS);
  pickHarmonic();

  // --- PHASE 2: FILL REMAINDER (Target Size 3 to 6) ---
  // Use forceSize if present, otherwise random
  const targetSize = forceSize ? forceSize : (Math.floor(Math.random() * (6 - 3 + 1)) + 3);
